...
# from playsound import playsound   # doesn't work on Android

import androidhelper.sl4a as sl4a   # only works on Android
android_sys = sl4a.Android()
    ...
    for _ in iter(int, 1): # infinite loop:
        ...
        if play != 'None':
            if not play in temp:
                ...
                # playsound(os.path.join('audio', play+'.mp3'))             # doesn't work on Android
                android_sys.mediaPlay(os.path.join('audio', play+'.mp3'))   # Only works on Android
                # ↑ doesn't play sound, when join parameter: play=True, program stocked.
